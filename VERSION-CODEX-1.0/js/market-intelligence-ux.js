...
function renderMarket() {
  ...
  mainContainer.innerHTML = `
    <section class="panel-card market-shell pro-market-shell">
      <div class="card-topline">
        <div>
          <span class="eyebrow">MARKET OPS · INTELLIGENCE UX</span>
          <h1 class="section-heading">Acciones Pro</h1>
        </div>
        <span class="status-pill green">FINNHUB</span>
      </div>
      <p class="hero-subtitle">Centro de inteligencia educativa con Finnhub live, sentimiento, scanner, watchlist, alertas, sectores, notas, sparkline y Market Agent local. No constituye asesoría financiera.</p>
      ${renderMarketApiCompact()}
    ...
    <section id="market-content" class="market-content">
      <div class="panel-card"><div class="loader small"></div><p class="hero-subtitle">Cargando inteligencia de mercado...</p></div>
    </section>
...
function renderMarketContent() {
  ...
  const selected = getQuoteBySymbol(marketState.detailSymbol) || null;
  ...
  if (statusBox) {
    statusBox.innerHTML = `<span class="market-dot ${marketState.status === 'live' ? 'online' : 'offline'}"></span><strong>${sourceLabel}</strong><small>${safeMarketEscape(marketState.source || 'Pendiente')} · ${formatMarketTime(marketState.updatedAt)}</small>`;
  }
  host.innerHTML = `
    ${renderMarketExecutiveSummary(sentiment)}
    ${renderMarketScorePanel(sentiment)}

    <section class="panel-card market-scanner">
      ${sectionTitle('Scanner del mercado', 'TOP')}
      <div class="scanner-grid">
        <div class="scanner-box"><span class="eyebrow">Top ganadoras</span>${gainers.map(q => renderMiniQuote(q)).join('')}</div>
        <div class="scanner-box"><span class="eyebrow">Top perdedoras</span>${losers.map(q => renderMiniQuote(q)).join('')}</div>
      </div>
    </section>

    ${renderAdvancedSectors(sentiment)}

    <section class="panel-card pro-watchlist-panel">
      ${sectionTitle('Mi Watchlist', `${watchQuotes.length} símbolos`)}
      ${watchQuotes.length ? `<div class="watchlist-strip pro-watchlist-strip">${watchQuotes.map(renderWatchChip).join('')}</div>` : `<div class="empty-state">Aún no sigues acciones. Toca una acción y pulsa ⭐ Seguir para crear tu lista.</div>`}
    </section>
    ...
    <section class="panel-card">
      ...
      <div class="stock-heatmap">${quotes.map(q => renderHeatTile(q, totalCap)).join('') || `<div class="empty-state">Sin resultados para el filtro actual.</div>`}</div>
    ...
    <section class="panel-card">
      ...
      <div class="stock-table">${quotes.map(renderStockRow).join('') || `<div class="empty-state">No hay acciones visibles.</div>`}</div>
    </section>
