...
function renderCourseDetail(courseId) {
  ...
  const exam = state.passedExams[course.id];
  const project = getCourseProject(course);
  mainContainer.innerHTML = `
...
function renderLesson(courseId, lessonId) {
  ...
  mainContainer.innerHTML = `<button class="btn btn-outline back-btn" onclick="renderView('course-detail', {courseId:'${courseId}'})">← SALIR</button><section class="panel-card lesson-hero-card animated-card"><span class="eyebrow">${escapeHtml(course.title)}</span><h1>${escapeHtml(lesson.title)}</h1><div class="chip-row">${chip(`Lección ${index}/${lessons.length}`)}${chip('+5 XP teoría','green')}${chip(`Lab ${lesson.xp} XP`,'orange')}${chip(`⏱ ${lesson.estimated_minutes} min`)}</div>${actionButton('INICIAR PRÁCTICA 💻', `openPracticeGuarded('${courseId}', '${lessonId}')`, 'btn-success', 'full top-practice')}</section><section class="panel-card animated-card"><h3 class="section-subtitle">Objetivo táctico</h3><p>${escapeHtml(lesson.objective)}</p><h3 class="section-subtitle">Teoría</h3><p class="theory-text">${escapeHtml(lesson.theory)}</p><div class="editor-wrapper"><div class="editor-header"><span>EJEMPLO.py</span><span>Referencia</span></div><pre class="code-view">${codeHtml(lesson.example_code)}</pre></div><div class="explanation-list">${lesson.example_explanation.map(text => `<div class="explain-item"><span>▸</span><p>${escapeHtml(text)}</p></div>`).join('')}</div></section><div class="bottom-spacer"></div>`;
}
...
async function runPythonSimulation(code, lesson, courseId) {
  ...
  try {
    ...
    if (!result || !result.ok) {
      ...
      showFeedback('error','Error explicado',result?.friendly || 'El simulador no pudo interpretar este código.');
      return;
    }
    ...
    if (passed) completeLesson(lesson, courseId);
      ...
      const details = getValidationDetails(output, lesson.exercise.expected_output);
      recordMistake({ courseId, lessonId: lesson.id, title: lesson.title, code, reason: details });
      showFeedback('warn','Salida incorrecta',details);
    }
  } catch (error) {
    consoleEl.textContent += `\n${error?.message || error}`;
...
function completeLesson(lesson, courseId) {
  ...
  showFeedback('success','✅ Correcto · misión completada', isNew ? `Salida validada. Ganaste ${lesson.xp} XP de laboratorio.` : 'Ya habías completado esta práctica.');
  renderQuiz(lesson);
...
function renderQuiz(lesson) {
  ...
  area.innerHTML = `<span class="eyebrow">Quiz rápido</span><h3>${escapeHtml(lesson.quiz.question)}</h3><div class="quiz-options">${lesson.quiz.options.map(opt => `<button class="btn btn-outline quiz-option" data-option="${escapeHtml(opt)}">${escapeHtml(opt)}</button>`).join('')}</div><p id="quiz-feedback"></p>`;
...
function showFeedback(type, title, message) { const el = document.getElementById('feedback-area'); el.className = `feedback ${type}`; el.innerHTML = `<strong>${escapeHtml(title)}</strong><p>${escapeHtml(message)}</p>`; el.classList.remove('hidden'); }
...
function renderProfile() {
  ...
  const rows = COURSES.map(course => { const stats = getCourseStats(course); const exam = state.passedExams[course.id]; return `<div class="profile-course-row" onclick="renderView('course-detail',{courseId:'${course.id}'})"><span>${course.icon} ${shortTitle(course.title)}</span><strong>${stats.completed}/${stats.total} <small>${exam?.passed ? '🏅' : stats.percent + '%'}</small></strong></div>`; }).join('');
  mainContainer.innerHTML = `<section class="welcome-card profile-hero animated-card"><span class="eyebrow">Perfil profesional</span><h1>Nivel ${state.level}</h1><p>XP, racha, rutas, exámenes, certificados, repaso, notas y respaldo seguro.</p>${progressBar(progress)}<div class="metric-grid">${metricCard(state.xp,'XP','glow-blue')}${metricCard(state.completedLessons.length,'Labs','glow-green')}${metricCard(state.streak,'Racha','glow-purple')}</div></section><section class="panel-card">${sectionTitle('Herramientas','Pro')}<div class="btn-row">${actionButton('REPASO', `renderView('review')`, 'btn-outline')}${actionButton('NOTAS', `renderView('notes')`, 'btn-outline')}${actionButton('TEMA', `toggleTheme()`, 'btn-outline')}</div></section><section class="panel-card">${sectionTitle('Respaldo y privacidad','JSON')}<p class="hero-subtitle">Exporta tu progreso local. Con protección activa, tus notas privadas no salen del dispositivo.</p><div class="check-list"><span>${state.privacyMode ? '🔐 Protección activa: notas excluidas al exportar' : '📝 Protección desactivada: exporta también notas'}</span></div><div class="btn-row">${actionButton('EXPORTAR', `exportProgressJSON()`, 'btn-primary')}${actionButton('IMPORTAR', `document.getElementById('import-progress-file').click()`, 'btn-outline')}${actionButton(state.privacyMode ? 'INCLUIR NOTAS' : 'PROTEGER NOTAS', `togglePrivacyMode()`, 'btn-outline')}</div></section><section class="panel-card">${sectionTitle('Certificados', state.certificates.length)}${state.certificates.length ? state.certificates.map(cert => `<div class="profile-course-row" onclick="renderView('certificate',{courseId:'${cert.courseId}'})"><span>🏅 ${shortTitle(cert.title)}</span><strong>${cert.percent}%</strong></div>`).join('') : '<p class="hero-subtitle">Aprueba exámenes de curso para desbloquear certificados locales.</p>'}</section><section class="panel-card">${sectionTitle('Rutas completadas', `${COURSES.length} rutas`)}<div class="profile-course-list">${rows}</div></section><section class="panel-card">${sectionTitle('Insignias', `${state.badges.length}/${BADGES.length}`)}<div class="badge-grid">${BADGES.map(b => `<div class="badge-item ${state.badges.includes(b.id)?'unlocked':''}"><div class="badge-icon">${b.icon}</div><div class="badge-name">${escapeHtml(b.title)}</div><p>${escapeHtml(b.description)}</p></div>`).join('')}</div></section><button class="btn btn-danger full" onclick="resetProgress()">ELIMINAR PERFIL Y DATOS</button><div class="bottom-spacer"></div>`;
}
